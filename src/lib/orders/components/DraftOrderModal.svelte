...
<script lang="ts">
  ...
  import { 
    ...
    Save, FileText, User, Hash
  } from 'lucide-svelte';
  import ProfileSelector from './ProfileSelector.svelte';
  import ProfileForm from '$lib/profiles/components/ProfileForm.svelte';
  ...
  function onProfileSelected(profileId: string, profileData: any) {
    const index = profiles.findIndex(p => p.id === profileId);
    if (index !== -1) {
      profiles[index].code = profileData.code;
      profiles[index].name = profileData.name;
      profiles = [...profiles];
    }
  }
      ...
      <div class="modal-body">
        ...
        <section class="form-section">
          <div class="section-header">
            ...
            {#if !readonly}
              <button class="add-profile-btn" on:click={addProfile} disabled={saving} type="button">
                <Plus size={16} />
                Add Profile
              </button>
            {/if}
            ...
            <div class="profile-card">
              <div class="profile-card-header">
                <ProfileSelector
                  ...
                  on:selected={e => onProfileSelected(profile.id, e.detail)}
                  disabled={readonly || saving}
                />
              ...
              {#if errors[`profile-${profile.id}`]}
                <span class="error-message">{errors[`profile-${profile.id}`]}</span>
              {/if}
            </div>
          {/each}
        </section>
